...
#define MAX_RATE 100
#define MIN_INTERVAL ((1.0/HEARTBEAT_HZ) > (1.0/MAX_RATE) ? (1.0/HEARTBEAT_HZ) : (1.0/MAX_RATE))
...
unsigned char var_channel_count; // Number of channels to be received
...
float GetBodyRates(float elapsedMe, float elapsedSim, int counter, void * refcon) {
    ...
    while (pendingElapsedTime >= MIN_INTERVAL) { // Don't run slower than MAX_RATE
        ...
        pendingElapsedTime -= (float) MIN_INTERVAL;
    }
